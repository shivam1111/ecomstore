...
def do_auth_capture(amount='0.00', card_num=None, exp_date=None,card_cvv=None):
    delimiter = '|'
    raw_params = {
        'x_login':config.AUTHNET_LOGIN,
        'x_tran_key':config.AUTHNET_KEY,
        'x_type':'AUTH_CAPTURE',
        'x_amount':amount,
        'x_version':'3.1', 
        'x_card_num':card_num,
    ...
    post_url = config.AUTHNET_POST_URL
    post_path = config.AUTHNET_POST_PATH
    print "***************************httplib.HTTPS_PORT",httplib.HTTPS_PORT
